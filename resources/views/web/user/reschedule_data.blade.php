@php 
    ...
    @foreach($RequestedPreferences as $pref)
        <div class="flight_search_items mt-3">
            <div class="multi_city_flight_lists pt-3">
                @foreach($pref['QuotedSegments'] as $segment)
                    ...
                    <div class="flight_multis_area_wrapper pb-4">
            ...
            <div class="flight_search_right  text-center h-100">
                    <h4> Total fare difference </h4>
                    <h2>{{ $currency }} {{ $fareDiff }} </h2>
                    <button type="submit" class="btn btn_theme btn_lg mt-10 reissueButton" data-uniqueID="{{ $uniqueBookId }}" data-ptrUniqueID="{{ $ptrUniqueId }}" data-currency="{{ $currency }}" data-fare="{{ $fareDiff }}" data-preference="{{ $pref['PreferenceOption'] }}" name="reissueButton" id="reissueButton">Send Request</button>
                </div>
        </div>
    @endforeach