                                        ...
                                        <div class="accordion-item">
                                            <h2 class="accordion-header">
                                                <button class="accordion-button collapsed" type="button"
                                                    ...
                                                    <div
                                                        class="d-flex gap-4 flex-wrap align-items-center justify-content-between">
                                                        <div class="d-flex align-items-center gap-4 flex-wrap">
                                                            <div
                                                                class="d-grid place-content-center w-12 h-12 box-shadow rounded-circle flex-shrink-0">
                                                                <div
                                                                    class="round-icon-p d-grid place-content-center w-10 h-10 bg-primary-50 clr-primary-300">
                                                                    <span class="icon-round-bb"> <i
                                                                            class="fas fa-plane-departure"></i> </span>
                                                                </div>
                                                            </div>
                                                            <div class="flex-grow-1">
                                                                <h5 class="fw-medium mb-1"> {{ $bookings->origin }} To
                                                                    {{ $bookings->destination }} </h5>
                                                                <ul
                                                                    class="list list-row align-items-center flex-wrap list-divider-half-xs">
                                                                    <li>
                                                                        <span class="d-inline-block fs-14">
                                                                            <span class="d-inline-block clr-neutral-500">
                                                                                Booking ID : </span>
                                                                            <span
                                                                                lass="d-inline-block clr-neutral-700 fw-medium">
                                                                    ...
                                                                    <div class="col-md-6">
                                                                        <div class="traveler-b-detl">
                                                                            ...
                                                                            <div class="form-check-label">
                                                                                ...
                                                                                <span class=" mb-2 fw-600 d-block">E-TICKET
                                                                                    NUMBER :
                                                                ...
                                                                @if (!empty($bookings['extraServices']))
                                                                    @php
                                                                        $desc = [
                                                                            ...
                                                                            'GROUP_PAX_INBOUND' =>
                                                                                '(Entire group only on the return trip)',
                                                                            'GROUP_PAX_OUTBOUND' =>
                                                                                '(Entire group only on for the onward travel)',
                                                                            'PER_PAX_INBOUND' =>
                                                                                '(Each passenger only on the return trip)',
                                                                            'PER_PAX_OUTBOUND' =>
                                                                                '(Each passenger only on for the onward travel)',
                                                                        ];
                                                                    @endphp
                            ...
                            <div class="tour_detail_right_sidebar mt-4">
                                <div class="tour_details_right_boxed">
                                    ...
                                    <div class="tour_booking_amount_area">
                                        <ul>
                                            @if ($bookings->adult_count != 0)
                                                <li>Adult Price x {{ $bookings->adult_count }}
                                                    <span>{{ $bookings->currency }} {{ $bookings->adult_amount }}</span>
                                                </li>
                                            @endif

                                            @if ($bookings->child_count != 0)
                                                <li>Child Price x {{ $bookings->child_count }}
                                                    <span>{{ $bookings->currency }} {{ $bookings->child_amount }}</span>
                                                </li>
                                            @endif

                                            @if ($bookings->infant_count != 0)
                                                <li>Infant Price x {{ $bookings->infant_count }}
                                                    <span>{{ $bookings->currency }} {{ $bookings->infant_amount }}</span>
                                                </li>
                                            @endif
                                        </ul>
                                        ...
                                        <div class="total_subtotal_booking">
                                            <h4>Total Amount <span>{{ $bookings->currency }}
                                                    {{ $bookings->total_amount }}</span> </h4>
                                        </div>
                                        <hr>
                                ...
                                @if ($bookings->parent_id != '')
                                    <div class="tour_details_right_boxed mt-4">
                                        ...
                                        <div class="tour_booking_amount_area">
                                            <div class="tour_bokking_subtotal_area">
                                                <h6>Old Booking Fare<span>{{ $bookings->currency }}
                                                        {{ $bookings->total_amount - $bookings->reschedule_fare_difference }}</span>
                                                </h6>
                                            </div>
                                            <div class="tour_bokking_subtotal_area">
                                                <h6>New Booking Fare Difference <span>{{ $bookings->currency }}
                                                        {{ $bookings->reschedule_fare_difference }}</span></h6>
                                            </div>
                                            <div class="total_subtotal_booking">
                                                <h4>Total Amount <span>{{ $bookings->currency }}
                                                        {{ $bookings->total_amount }}</span> </h4>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <button type="button" class="btn btn-success" id="requestCancel">Send Cancel
                                            Request</button>
                    ...
                    if (resp.status == true) {
                        if (resp.type == 'cancel') {
                            ...
                            if (resp.type == 'refund') {
                                ...
                                $('#totalCancelFee').html(resp.data.currency + ' ' + resp.data
                                    .refundFee);
                            } else {
                            ...
                            $('#serviceCharge').html(resp.data.currency + ' ' + resp.data
                                .serviceCharge);
