<div class="flight_search_result_wrapper" id="return_trip">
    @if ($data['flightDetails'])
        ...
        @php
            ...
            $from_flight = Arr::where($data['flightDetails'], function ($value, $key) use ($startRoute) {
                if ($value['api_provider'] == 'yasin') {
                    return $value['Route'] == $startRoute;
                } elseif ($value['api_provider'] == 'flydubai') {
                    return $value['leg_count'] == 1;
                }
            });

            $to_flight = Arr::where($data['flightDetails'], function ($value, $key) use ($startRoute, $endRoute) {
                if ($value['api_provider'] == 'yasin') {
                    return $value['Route'] == $endRoute;
                } elseif ($value['api_provider'] == 'flydubai') {
