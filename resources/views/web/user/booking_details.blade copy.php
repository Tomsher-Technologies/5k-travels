                    ...
                    <div id="tab-booking-details">
                        ...
                        <div class="mt-n5 d-flex gap-3 flex-wrap align-items-end">
                            <div>
                                ...
                                @php
                                    $childId = '';
                                    if($bookings->is_reissued == 1){
                                        $childId = getNewReissuedBooking($bookings->id);
                                        $childId = isset($childId[0]) ? $childId[0] : $bookings->id;
                                    }
                                @endphp
                            </div>
                            ...
                            <div class="ms-md-auto">
                                @if($type == 'my_bookings' || $type == 'upcoming')
                                    ...
                                    @php   
                                        $timeDiff = time() - strtotime($bookings->created_at);
                                    @endphp
                                    @if($bookings->reissue_request == 0)
                                        @if($bookings->is_cancelled == 0 && $bookings->cancel_request == 0)
                                            @if($timeDiff < 86400)
                                                @if(strtolower($bookings->fare_type) == 'webfare' )
                                                    <a href="javascript:void(0)" class="refundQuoteTicket btn btn-cancel" title="Cancel Ticket" id=""  data-bookid="{{ $bookings->id }}" data-id="{{ $bookings->unique_booking_id }}">Cancel Booking</a>
                                                @else
                                                    @if($bookings->ticket_status == "Ticketed" || $bookings->ticket_status == "OK")
                                                        <a href="javascript:void(0)" class="voidQuoteTicket btn btn-cancel" title="Cancel Ticket" id="" data-provider="{{ $bookings->api_provider }}" data-bookid="{{ $bookings->id }}" data-id="{{ $bookings->unique_booking_id }}">Cancel Booking</a>
                                                    @else
                                                        <a href="javascript:void(0)" class="cancelTicket btn btn-cancel" title="Cancel Ticket" data-type="void" id="" data-provider="{{ $bookings->api_provider }}" data-bookid="{{ $bookings->id }}" data-id="{{ $bookings->unique_booking_id }}">Cancel Booking</a>
                                                    @endif
                                                @endif
                                            @elseif($timeDiff > 86400)
                                                ...
                                                @else
                                                    @if($bookings->ticket_status == "Ticketed" || $bookings->ticket_status == "OK")
                                                        <a href="javascript:void(0)" class="refundQuoteTicket btn btn-cancel" title="Cancel Ticket" id="" data-bookid="{{ $bookings->id }}" data-id="{{ $bookings->unique_booking_id }}">Cancel Booking</a>
                                                    @else
                                                        <a href="javascript:void(0)" class="cancelTicket btn btn-cancel" data-type="refund" title="Cancel Ticket" id="" data-bookid="{{ $bookings->id }}" data-id="{{ $bookings->unique_booking_id }}">Cancel Booking</a>
                                                    @endif
                                                @endif
                                            @endif
                                        @elseif($bookings->is_cancelled == 0 && $bookings->cancel_request == 1)
                                            <a href="javascript:void(0)" class="btn btn-info cancelPTRStatusCheck" title="Check Cancel Status" data-id="{{ $bookings->unique_booking_id }}" data-ptr="{{ $bookings->cancel_ptr }}">Check Cancel Status</a> 
                                        @elseif($bookings->is_cancelled == 1)
                                            <span class="badge bg-danger line-height-badge"> Cancelled </span>
                                        @endif
                                    @endif

                                    @if($bookings->cancel_request == 0)
                                        @if($bookings->is_reissued == 0 && $bookings->reissue_request == 1)
                                            <a href="javascript:void(0)" class="btn btn-info reissuePTRStatusCheck" title="Check Cancel Status" data-id="{{ $bookings->id }}" data-uniqueid="{{ $bookings->unique_booking_id }}"  data-ptr="{{ $bookings->reissue_ptr }}">Check Reissue Status</a> 
                                        @elseif($bookings->is_reissued == 1)
                                            <a href="{{ route('booking-details', ['type' => $type, 'id' => $childId] ) }}">
                                                <span class="badge bg-info line-height-badge"> Rescheduled </span>
                                            </a>
                                        @else
                                            <a href="{{ route('change-date',['type' => $type,'id' => $bookings->id,'unique_id' => $bookings->unique_booking_id] ) }}" class="btn btn-warning ">Reschedule Booking</a>
                                        @endif
                                    @endif
                                @else
                                                ...
                                                <div class="d-flex gap-4 flex-wrap align-items-center justify-content-between">
                                                    <div class="d-flex align-items-center gap-4 flex-wrap">
                                                        ...
                                                        <div class="flex-grow-1">
                                                            <h5 class="fw-medium mb-1"> {{ $bookings->origin }} To {{ $bookings->destination }} </h5>
                                                                    ...
                                                                    <div class="relative z-0">
                                                                        ...
                                                                        <div class="d-flex items-center">
                                                                            ...
                                                                            <div class="row">
                                                                                ...
                                                                                <div class="col-auto">
                                                                                    <div class="lh-14 fw-500">{{ $deptAirportData['AirportName'] }}, {{ $deptAirportData['City'] }} ({{ $flights->departure_airport }})</div>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                        ...
                                                                        <div class="d-flex items-center mt-15">
                                                                            ...
                                                                            <div class="row">
                                                                                ...
                                                                                <div class="col-auto">
                                                                                <div class="lh-14 fw-500">{{ $arrAirportData['AirportName'] }}, {{ $arrAirportData['City'] }} ({{ $flights->arrival_airport }})</div>
                                                                                </div>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                        ...
                        <div class="tour_detail_right_sidebar mt-4">
                                ...
                                <div class="tour_booking_amount_area">
                                    <ul>
                                        @if($bookings->adult_count != 0)
                                            <li>Adult Price x {{ $bookings->adult_count }} <span>{{ $bookings->currency }} {{ $bookings->adult_amount }}</span></li>
                                        @endif

                                        @if($bookings->child_count != 0)
                                            <li>Child Price x {{ $bookings->child_count }} <span>{{ $bookings->currency }} {{ $bookings->child_amount }}</span></li>
                                        @endif

                                        @if($bookings->infant_count != 0)
                                            <li>Infant Price x {{ $bookings->infant_count }} <span>{{ $bookings->currency }} {{ $bookings->infant_amount }}</span></li>
                                        @endif
                                    </ul>
                                    <div class="tour_bokking_subtotal_area">
                                        <h6>Total Base Fare <span>{{ $bookings->currency }} {{ $bookings->adult_amount + $bookings->child_amount + $bookings->infant_amount  }}</span></h6>
                                    </div>
                                    <div class="tour_bokking_subtotal_area">
                                        <h6>Total Tax <span>{{ $bookings->currency }} {{ $bookings->total_tax }}</span></h6>
                                    </div>

                                    <div class="tour_bokking_subtotal_area">
                                        <h6>Add Ons <span id="addons">{{ $bookings->currency }} {{ $bookings->addon_amount }}</span></h6>
                                    </div>
                                    ...
                                    <div class="total_subtotal_booking">
                                        <h4>Total Amount <span >{{ $bookings->currency }} {{ $bookings->total_amount }}</span> </h4>
                                    </div>
                                    <hr>
                            ...
                            <div class="tour_details_right_boxed mt-4">
                                ...
                                <div class="tour_booking_amount_area">
                                    <div class="tour_bokking_subtotal_area">
                                        <h6>Old Booking Fare<span>{{ $bookings->currency }} {{ $bookings->total_amount - $bookings->reschedule_fare_difference }}</span></h6>
                                    </div>
                                    <div class="tour_bokking_subtotal_area">
                                        <h6>New Booking Fare Difference <span>{{ $bookings->currency }} {{ $bookings->reschedule_fare_difference }}</span></h6>
                                    </div>
                                    <div class="total_subtotal_booking">
                                        <h4>Total Amount <span >{{ $bookings->currency }} {{ $bookings->total_amount }}</span> </h4>
                                    </div>
